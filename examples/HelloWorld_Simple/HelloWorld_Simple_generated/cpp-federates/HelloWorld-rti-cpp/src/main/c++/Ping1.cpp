...
void Ping1::init( RTI::RTIambassador *rti ) {
	...
	while( isNotInitialized ) {
		try {
			getHandle() = rti->getInteractionClassHandle( "InteractionRoot.C2WInteractionRoot.Ping1" );
	...
	getClassNameHandleMap().insert(  std::make_pair( "Ping1", get_handle() )  );
	getClassHandleNameMap().insert(  std::make_pair( get_handle(), "Ping1" )  );
	...
	getDatamemberNameHandleMap().insert(  std::make_pair( "Ping1,Count", get_Count_handle() )  );
...
void Ping1::publish( RTI::RTIambassador *rti ) {
	if ( getIsPublished() ) {
		return;
	}
...
void Ping1::unpublish( RTI::RTIambassador *rti ) {
	if ( !getIsPublished() ) {
		return;
	}
...
void Ping1::subscribe( RTI::RTIambassador *rti ) {
	if ( getIsSubscribed() ) {
		return;
	}
...
void Ping1::unsubscribe( RTI::RTIambassador *rti ) {
	if ( !getIsSubscribed() ) {
		return;
	}
...
bool Ping1::static_init( void ) {
	...
	getClassNameSet().insert( "Ping1" );
	
	getClassNameFactoryMap().insert(  std::make_pair( "Ping1", &Ping1::factory )  );
	getClassNamePublishMap().insert(   std::make_pair(  "Ping1", (PubsubFunctionPtr)( &Ping1::publish )  )   );
	getClassNameUnpublishMap().insert(   std::make_pair(  "Ping1", (PubsubFunctionPtr)( &Ping1::unpublish )  )   );
	getClassNameSubscribeMap().insert(   std::make_pair(  "Ping1", (PubsubFunctionPtr)( &Ping1::subscribe )  )   );
	getClassNameUnsubscribeMap().insert(   std::make_pair(  "Ping1", (PubsubFunctionPtr)( &Ping1::unsubscribe )  )   );

	getDatamemberClassNameVectorPtrMap().insert(  std::make_pair( "Ping1", &getDatamemberNames() )  );
	getAllDatamemberClassNameVectorPtrMap().insert(  std::make_pair( "Ping1", &getAllDatamemberNames() )  );
...
std::ostream &operator<<( std::ostream &os, Ping1::SP entitySP ) {
	return os << *entitySP;
}
std::ostream &operator<<( std::ostream &os, const Ping1 &entity ) {
	return os << "Ping1("  << "sourceFed:" << entity.get_sourceFed() << ", " << "originFed:" << entity.get_originFed() << ", " << "federateFilter:" << entity.get_federateFilter() << ", " << "Count:" << entity.get_Count() << ", " << "actualLogicalGenerationTime:" << entity.get_actualLogicalGenerationTime()	<< ")";
}
...
Ping1::ParameterHandleValuePairSetSP Ping1::createDatamemberHandleValuePairSet( RTI::ULong count ) {
